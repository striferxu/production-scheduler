...
> 🐳 **Docker 部署**：项目根目录已内置 `Dockerfile` + `docker-compose.yml`。若你环境有 Docker，建议直接用容器方式（**更干净、隔离、迁移方便**，数据库持久化在命名卷）。详见项目根目录 [`README.md`](../README.md) 的「Docker 部署」章节，或直接：
> ```bash
> docker compose up -d --build
> ```
> 本文件以下内容为**原生 systemd 部署**（不依赖 Docker）的完整说明。
